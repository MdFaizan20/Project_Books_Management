...
const getBook = async function (req, res) {
    ...
    try {
        ...
        if (req.query.userId || req.query.category || req.query.subCategory) {
            ...
            const gettingBooks = await booksModel.findOne(obj)
            if (!gettingBooks) {
...
//----------------------book details with review(Get api)------
...
const updateBook = async function (req, res) {
    ...
    try {
        ...
        if (!valid.isValid(title)) {
            return res.status(400).send({ status: false, msg: "pls provide tittle for the book" })
        }
        if (!valid.isValid(excerpt)) {
            return res.status(400).send({ status: false, msg: "pls write excerpt" })
        }
...
const removeBook = async function (req, res) {
    try {
        ...
        if(!removeData){
            return res.status(404).send({status:false,msg:"pls provide valid book Id"})
        }
