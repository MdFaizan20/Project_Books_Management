...
const removeReview = async function (req, res) {
    try {
        ...
        if (!book) {
            res.status(400).send({ status: false, message: "pls provide  book id" })
        ...
        if (!reviewId) {
            return res.status(404).send({ status: false, msg: " Invalid review Id" })
        }
